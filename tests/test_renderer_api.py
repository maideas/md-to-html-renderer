...
from github_markdown.renderer import STRUCTURE_NAME
...
class TestStylesheetCoverage:
    ...
    def test_every_pygments_token_class_is_styled(self):
        css = Path(GitHubMarkdown.structure_css()).read_text(encoding="utf-8")
    ...
    def test_no_css_variable_is_used_without_being_defined(self):
        ...
        css = "".join(
            path.read_text(encoding="utf-8") for path in GitHubMarkdown().asset_paths()
        )
    ...
    def test_page_shell_only_uses_tokens_the_palette_defines(self):
        ...
        tokens are renamed."""
        from github_markdown.templates import PAGE_SHELL_CSS
    ...
    def test_every_palette_covers_all_three_theme_states(self, name):
        css = GitHubMarkdown(RenderOptions(palette=name)).stylesheets()
